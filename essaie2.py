...
button_command = "clique sur allumer la caméra"
...
if start : 
    ...
    out = cv.VideoWriter('C:/Users/laura/.env/webmining/video_output.mp4', cv.VideoWriter_fourcc(*'mp4v'), 5, (frame_width,frame_height))
    face_names = []
    ...
    while cv.waitKey(1) < 0:
        ...
        for bbox in faces:
            ...
            ## Création du carré
            cv.rectangle(frameFace, (bbox[1], bbox[0]), (bbox[3], bbox[2]), (0, 255, 0), int(round(frameFace.shape[0]/150)), 8)
            ...
            for i, line in enumerate(label.split(',')):
                ...
                try:
                    ...
                    if line in ["Enervé", "Dégouté", "Peureux", "Heureux", "Neutre", "Triste","Surpris"]:
                        cv.putText(frameFace, line, (bbox[1] + 6,  bbox[0] - 6 + k), font, 1.0, (255, 255, 255), 1)
                    elif line.startswith("("):
                        cv.putText(frameFace, line+" "+str(round(max(agePreds[0].tolist())*100,2)), (bbox[1] + 6,  bbox[0] - 6 + k), font, 1.0, (0, 0, 0), 1)
                    elif line == "Male":
                        cv.putText(frameFace, line+" "+str(round(max(genderPreds[genderPreds[0].argmax()].tolist())*100,2)), (bbox[1] + 6,  bbox[0] - 6 + k), font, 1.0, (255, 255, 255), 1)
                    elif line == "Female":
                        cv.putText(frameFace, line, (bbox[1] + 6,  bbox[0] - 6 + k), font, 1.0, (255, 255, 255), 1)
                    else:
                        cv.putText(frameFace, line, (bbox[1] + 6,  bbox[0] - 6 + k), font, 1.0, (0, 0, 0), 1)
                except:
                    pass
